...
#define WITH_GTSAM 0  // with GTSAM
 ...
 * @return
 */
Eigen::Matrix<double, kMeasDim, kStateDim> MAPFusionNode::measurementH(const Eigen::Quaterniond &vo_q,
  ...
  Eigen::Quaterniond q(map_ptr_->state_ptr_->Rwb_);

  switch (map_ptr_->kJacobMeasurement_) {
...
void MAPFusionNode::check_jacobian(const Eigen::Quaterniond &vo_q, const Eigen::Isometry3d &Twb) {
